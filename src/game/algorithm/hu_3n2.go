...
func detectAllTypeHu(cs []byte, ch, ps, ks []uint32, wildcard byte) int64 {
	if hu3n2(cs) {
...
func existHu3n2(cs []byte, ch, ps, ks []uint32, wildcard byte) int64 {
	hu := detectAllTypeHu(cs, ch, ps, ks, wildcard)
	if hu > 0 {
		return hu
	}
	// 财神是白板的情况作限制
		...
		if baicount > 0 {
			...
			for k, v := range cs {
				if v == BAI {
					...
					hu := detectAllTypeHu(cards, ch, ps, ks, wildcard)
					if hu > 0 {
						return hu
					}
				}
